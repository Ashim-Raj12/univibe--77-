...
const GradientAvatar: React.FC<{ src?: string | null, alt: string, size?: string }> = ({ src, alt, size = 'h-10 w-10' }) => (
    ...
    return (
        <div className="group flex items-start gap-3 py-2">
            <Link to={`/profile/${comment.profiles?.id}`}>
                <GradientAvatar src={comment.profiles?.avatar_url} alt={comment.user_id} size="h-10 w-10" />
            </Link>
            <div className="flex-1">
                <div className="bg-dark-card rounded-xl px-3 py-2 inline-block">
                    <div className="flex items-center gap-1">
                        <Link to={`/profile/${comment.profiles?.id}`} className="font-bold text-sm text-text-heading hover:underline">
                            {comment.profiles?.name}
                        </Link>
                        {comment.profiles && <VerifiedBadge profile={comment.profiles} size="h-4 w-4" />}
                    </div>
                    {replyInfo && (
                        <div className="border-l-2 border-primary/50 pl-2 mt-2 text-xs">
                            <Link to={`/profile/${replyInfo.senderId}`} className="font-semibold text-text-body hover:underline">
                                {replyInfo.senderName}
                            </Link>
                            <p className="text-text-muted truncate">{replyInfo.content}</p>
                        </div>
                    )}
                    {mainContent && <p className="text-text-body text-sm whitespace-pre-wrap mt-1">{displayContent}</p>}
                    {isLongComment && (
                        <button
    ...
    return (
        <form onSubmit={handleSubmit} className="pt-4">
            ...
            <div className="flex items-start gap-3">
                <GradientAvatar src={profile?.avatar_url} alt={user?.id || 'user'} size="h-10 w-10"/>
                <div className="w-full relative flex items-center">
                    ...
                    />
                    <button type="submit" disabled={loading || !content.trim()} className="absolute right-1.5 top-1/2 -translate-y-1/2 text-white bg-primary disabled:bg-slate-500 hover:bg-primary-focus transition-colors rounded-full p-1.5 active:animate-press">
                    ...
                    if (error) {
                        ...
                        if (commentWithProfile.profiles) {
                            const { data: proSub } = await supabase
                                ...
                                .eq('status', 'active')
                                .maybeSingle();
                            if(proSub && proSub.subscriptions?.name?.toUpperCase() === 'PRO'){
            ...
            .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'comments', filter: `post_id=eq.${postId}` },
              async (payload) => {
                ...
                if (commentWithProfile && !error) {
                    ...
                    setComments(current => current.map(c => c.id === updatedComment.id ? finalComment : c));
                }
              }
            )
            .on(
