...
import { StudyGroupWithMemberCount, StudyGroupInviteWithDetails } from '../types';
        ...
        <div className="bg-blue-50 border-l-4 border-blue-400 p-4 rounded-r-lg flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 animate-fade-in-up">
            <div>
                <p className="text-sm text-blue-800">
                    You've been invited to join <strong className="font-bold">{invite.study_groups.name}</strong> by <strong className="font-bold">{invite.profiles.name}</strong>.
                </p>
            </div>
            <div className="flex gap-2 flex-shrink-0">
                <button onClick={() => onAccept(invite.id)} disabled={actionLoading} className="bg-primary text-white px-4 py-1.5 rounded-lg text-sm font-semibold hover:bg-primary-focus transition-colors disabled:opacity-50 min-w-[80px] flex justify-center">
                    {actionLoading ? <Spinner size="sm" /> : 'Accept'}
                </button>
                <button onClick={() => onDecline(invite.id)} disabled={actionLoading} className="bg-slate-200 text-text-body px-4 py-1.5 rounded-lg text-sm font-semibold hover:bg-slate-300 transition-colors disabled:opacity-50">
                    Decline
                </button>
            </div>
        </div>
...
const StudyGroupsTab: React.FC = () => {
    ...
    const [discoverGroups, setDiscoverGroups] = useState<StudyGroupWithMemberCount[]>([]);
    ...
    const fetchGroups = useCallback(async () => {
        ...
        try {
            ...
            let enrichedInvites = (invitesData as StudyGroupInviteWithDetails[]) || [];
            const inviterIds = enrichedInvites.map(i => i.inviter_id);
            if (inviterIds.length > 0) {
                 const { data: proSubs } = await supabase.from('user_subscriptions').select('user_id, subscriptions:subscription_id(name)').in('user_id', inviterIds).eq('status', 'active');
                 ...
                 enrichedInvites = enrichedInvites.map(invite => ({
                    ...invite,
                    profiles: { ...invite.profiles, has_pro_badge: proUserIds.has(invite.inviter_id) }
                 }));
            }
            ...
            const { data: discoverData, error: discoverError } = await discoverQuery.order('name');
            if (discoverError) throw discoverError;
            setDiscoverGroups(discoverData || []);

        } catch (e: any) {
            console.error("Error fetching study groups:", e);
            setError(`Failed to fetch study groups. This might be due to a database security rule. Error: ${e.message}`);
        } finally {
            setLoading(false);
        }
    }, [user, profile]);
    ...
    useEffect(() => {
        ...
        const channel = supabase
            ...
            .on('postgres_changes', { event: '*', schema: 'public', table: 'study_group_members', filter: `user_id=eq.${user.id}` }, fetchGroups)
            .on('postgres_changes', { event: '*', schema: 'public', table: 'study_group_invites', filter: `invitee_id=eq.${user.id}` }, fetchGroups)
            .subscribe();
    ...
    const handleAcceptInvite = async (inviteId: number) => {
        ...
        const { error } = await supabase.rpc('accept_study_group_invite', { p_invite_id: inviteId });
        if (error) {
                ...
                <>
                    {invites.length > 0 && (
                        <div className="space-y-3">
                            <h3 className="text-lg font-bold text-text-heading">Pending Invitations</h3>
                            {invites.map(invite => (
                                <InviteCard 
                                    key={invite.id} 
                                    invite={invite} 
                    ...
                    <div className="pt-4">
                        <h2 className="text-xl font-bold text-text-heading">Discover Groups</h2>
                    </div>
