...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
    ...
    const [mutatingFriendshipIds, setMutatingFriendshipIds] = useState<Set<string>>(new Set());
    ...
    const fetchFriendships = useCallback(async (userToFetch: User | null) => {
        ...
        if (friendshipsData) {
            ...
            console.error("Error fetching friendships:", JSON.stringify(friendshipsError, null, 2));
        }
        setFriendshipsLoading(false);
    }, []);
    ...
    const fetchCoreUserData = useCallback(async (userToFetch: User | null) => {
        ...
        if (subscriptionError) {
            console.error('Error fetching subscription:', JSON.stringify(subscriptionError, null, 2));
        }

        if (profileData) {
            // Special parent override (keep this logic)
            const parentOverrideEmails = ['rituraj0gupta@gmail.com', 'sumit129@gmail.com'];
            if (userToFetch?.email && parentOverrideEmails.includes(userToFetch.email)) {
            ...
            console.error(`CRITICAL: Profile not found for user ${userToFetch.id}. The signup trigger likely failed catastrophically.`);
        }
        ...
        const { data: authListener } = supabase.auth.onAuthStateChange(
            (_event, newSession) => {
                ...
                if (!currentUser) { // SIGNED_OUT
                    ...
                    setWallet(null);
                    setFollowingIds(new Set());
                } else { // SIGNED_IN or other events
    ...
    useEffect(() => {
        ...
        const walletChannel = supabase
            ...
            .on('postgres_changes', { event: '*', schema: 'public', table: 'vibecoin_wallets', filter: `user_id=eq.${user.id}` }, 
            payload => {
    ...
    const toggleFollow = useCallback(async (userId: string) => {
        ...
        try {
            const { error } = await supabase.rpc('toggle_follow', { p_follower_id: user.id, p_following_id: userId });
            ...
            alert('An unexpected error occurred while trying to follow. Please try again.');
            throw e;
        } finally {
    ...
    const value: AuthContextType = useMemo(() => ({
        ...
        friendshipsLoading,
    }), [session, user, profile, subscription, wallet, loading, signOut, refetchProfile, refetchWallet, isFollowing, toggleFollow, mutatingFriendshipIds, friendshipsLoading]);
