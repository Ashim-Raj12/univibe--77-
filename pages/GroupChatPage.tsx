...
import { StudyGroup, StudyGroupMessage, StudyGroupMessageWithProfile, Profile, ReplyInfo, StudyGroupInvite } from '../types';
...
const ChatMessage: React.FC<{ message: StudyGroupMessageWithProfile; isSender: boolean; onDelete: (messageId: number) => void; onReply: (message: StudyGroupMessageWithProfile) => void; onEdit: (message: StudyGroupMessageWithProfile) => void; }> = ({ message, isSender, onDelete, onReply, onEdit }) => {
    ...
    return (
        <div className={`group relative flex w-full ${isSender ? 'justify-end' : 'justify-start'}`}>
            <div className={`flex items-start gap-3 ${isSender ? 'flex-row-reverse' : ''}`}>
                ...
                <div className={`flex flex-col max-w-xs md:max-w-md ${isSender ? 'items-end' : 'items-start'}`}>
                    ...
                    <div className="relative"> {/* Container for bubble + menu */}
                        <div className={`p-3 rounded-xl ${messageClasses}`} onClick={handleMessageClick}>
                            {replyInfo && (
                        ...
                        <div ref={menuRef} className={`absolute top-1/2 -translate-y-1/2 flex gap-1 bg-white border rounded-full shadow-md p-1 transition-all duration-200 opacity-0 md:group-hover:opacity-100 scale-90 md:group-hover:scale-100 ${isMenuOpen ? '!opacity-100 !scale-100' : ''} ${isSender ? 'left-0 -translate-x-full -ml-2 origin-left' : 'right-0 translate-x-full mr-2 origin-right'}`}>
                            <button onClick={() => { onReply(message); setIsMenuOpen(false); }} className="p-1 rounded-full hover:bg-slate-200" title="Reply"><svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M7.707 3.293a1 1 0 010 1.414L5.414 7H11a7 7 0 017 7v2a1 1 0 11-2 0v-2a5 5 0 00-5-5H5.414l2.293 2.293a1 1 0 11-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clipRule="evenodd" /></svg></button>
                            {isSender && (
                                <>
                                <button onClick={() => { onEdit(message); setIsMenuOpen(false); }} className="p-1 rounded-full hover:bg-slate-200" title="Edit"><svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor"><path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" /></svg></button>
                                <button onClick={() => { onDelete(message.id); setIsMenuOpen(false); }} className="p-1 rounded-full hover:bg-slate-200" title="Delete"><svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z" clipRule="evenodd" /></svg></button>
                                </>
                            )}
                        </div>
                    </div>
...
const GroupChatPage: React.FC = () => {
    ...
    const [pendingInvite, setPendingInvite] = useState<StudyGroupInvite | null>(null);
    ...
    const fetchData = useCallback(async (shouldReloadMessages = true) => {
        ...
        const isUserMember = study_group_members?.some(m => m.user_id === user.id) ?? false;
        setIsMember(isUserMember);
        
        const memberProfilesRaw = study_group_members?.map(m => m.profiles).filter(Boolean) as Profile[] || [];
        ...
        if (!isUserMember) {
            const { data } = await supabase.from('study_group_invites').select('*').eq('group_id', groupId).eq('invitee_id', user.id).eq('status', 'pending').maybeSingle();
            inviteData = data;
        ...
        if (shouldReloadMessages && (isUserMember || groupDetails.type === 'public')) {
    ...
    useEffect(() => {
        if (!user || isNaN(groupId) || (group?.type === 'private' && !isMember && !pendingInvite)) return;
        
        const messageChannel = supabase.channel(`group-chat-${groupId}`, { config: { presence: { key: currentUserProfile?.name || user.id } } })
        ...
        async (payload: any) => {
            ...
            if (!senderProfile) {
                ...
                if (newProfile && !error) {
                    ...
                    console.error(`Could not fetch profile for user ${newMessageData.user_id}`, error);
                    return;
                }
            }
            ...
            setMembers(current => {
                const existingIndex = current.findIndex(p => p.id === senderProfile!.id);
                if (existingIndex > -1) {
            ...
            const newMessageWithProfile: StudyGroupMessageWithProfile = { ...newMessageData, profiles: senderProfile };
            setMessages(current => current.some(m => m.id === newMessageWithProfile.id) ? current : [...current, newMessageWithProfile]);
        })
        .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'study_group_messages', filter: `group_id=eq.${groupId}` }, (payload: any) => {
        ...
        .on('postgres_changes', { event: 'DELETE', schema: 'public', table: 'study_group_messages', filter: `group_id=eq.${groupId}` }, (payload: any) => setMessages(current => current.filter(m => m.id !== (payload.old as { id: number }).id)))
        ...
        .subscribe(async (status) => {
            if (status === 'SUBSCRIBED') {
                ...
                await messageChannel.track({ is_typing: false, name: currentUserProfile?.name || user.id });
            }
        });

        const membersChannel = supabase.channel(`group-members-${groupId}`)
    ...
    }, [groupId, user, isMember, group?.type, currentUserProfile, fetchData, pendingInvite]);
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        ...
        if (group?.type === 'public' && !isMember) {
            ...
            if (joinError && joinError.code !== '23505') { // 23505 = unique_violation (race condition)
                toast.error("Could not join group to send message: " + joinError.message);
                setIsSending(false);
                return;
            }
        ...
        if (channelRef.current && currentUserProfile.name) {
            await channelRef.current.track({ is_typing: false, name: currentUserProfile.name });
        }
        ...
        try {
            ...
            if (tempFile) {
                const filePath = `${user.id}/group-chats/${groupId}/${Date.now()}_${tempFile.name}`;
            ...
            const { data: insertedMessage, error } = await supabase.from('study_group_messages').insert({ 
                group_id: groupId, 
                user_id: user.id, 
                content: finalContent, 
                file_url: fileUrl, 
            ...
            const newMessageWithProfile: StudyGroupMessageWithProfile = { ...(insertedMessage as StudyGroupMessage), profiles: currentUserProfile };
            setMessages(current => current.map(m => m.id === tempId ? newMessageWithProfile : m));
    ...
    const handleAcceptInvite = async () => {
        ...
        const { error } = await supabase.rpc('accept_study_group_invite', { p_invite_id: pendingInvite.id });
    ...
    if (!group || !currentUserProfile) return <p className="text-center p-8">Group not found.</p>;
    if (group.type === 'private' && !isMember && !pendingInvite) {
         return (
            <div className="flex flex-col h-full items-center justify-center text-center p-4">
                <h2 className="text-2xl font-bold">Private Group</h2>
                <p className="text-text-muted mt-2">You must be a member to view this group.</p>
                <button onClick={() => navigate('/study-hub')} className="mt-4 bg-primary text-white px-4 py-2 rounded-lg font-semibold">Back to Study Hub</button>
            </div>
         );
    }
        ...
        <div className="flex h-full max-w-6xl mx-auto">
            <div className="flex flex-col flex-1 bg-card rounded-2xl shadow-soft border border-slate-200/50">
                <header className="p-4 border-b border-slate-200 flex items-center justify-between gap-3">
                    ...
                    <div className="flex-shrink-0">{isMember && group.type === 'private' && (<button onClick={() => setIsInviteModalOpen(true)} className="text-sm font-semibold text-primary whitespace-nowrap">Invite Members</button>)}</div>
                </header>
                {pendingInvite && !isMember && (
                    <div className="p-3 bg-blue-100 border-b border-blue-200 text-center animate-fade-in-up">
                        <p className="font-semibold text-blue-800 text-sm">You have been invited to join this group.</p>
                        <div className="flex gap-3 justify-center mt-2">
                            <button onClick={handleAcceptInvite} disabled={inviteActionLoading} className="bg-primary text-white px-5 py-1.5 rounded-lg font-semibold hover:bg-primary-focus transition-colors text-sm">
                                {inviteActionLoading ? <Spinner size="sm"/> : 'Accept'}
                            </button>
                            <button onClick={handleDeclineInvite} disabled={inviteActionLoading} className="bg-slate-200 text-text-body px-5 py-1.5 rounded-lg font-semibold hover:bg-slate-300 transition-colors text-sm">
                                Decline
                            </button>
                        </div>
                    </div>
                ...
                {isMember || group.type === 'public' ? (
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
                        <h2 className="text-2xl font-bold text-text-heading">Join the Group!</h2>
                        <p className="text-lg text-text-muted mt-2">
                           {pendingInvite ? 'Accept the invitation above to start chatting.' : 'This is a private group.'}
                        </p>
                    </div>
                )}
            </div>
            ...
            {isEditModalOpen && isCreator && <EditGroupModal group={group} onClose={() => setIsEditModalOpen(false)} onSuccess={() => fetchData(false)} onDelete={handleGroupDeleted} />}
            {isInviteModalOpen && isMember && <InviteFriendsModal group={group} members={members} onClose={() => setIsInviteModalOpen(false)} />}
        </div>
    );
};
